...
def insert_data_to_database(sqls, tablename):
    """
    Write data to the database, if there is an error, send mail...
    :param sqls: FORMAT ({...},{...},{...})
    """
    ...
    try:
        ...
        content_sub = "{} Faled...".format(tablename) + " " * 4 + " " * 4 + "邮件" + datetime.datetime.now().strftime("%H:%M:%S")
        mail(content_sub, e)
        raise e
...
def check_whether_the_data_is_complete(tables):
    ...
    if insert_success_lst:
        flag = "Table inserted successfully" + datetime.datetime.now().strftime("%H:%M:%S")
        mail(flag, insert_success_lst)
    if insert_failed_lst:
        flag = "Insert failed table" + datetime.datetime.now().strftime("%H:%M:%S")
...
if __name__ == "__main__":
    ...
    check_whether_the_data_is_complete(all_table_data)
