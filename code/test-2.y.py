...
def insert_marketdatabase(path,connection):
    ...
    data=connection.loadTable("dolphindb_share_table").toDF()
    connection.run("append!{{loadTable('{db}', `{tb})}}".format(db='dfs://marketdata',tb='symbol_data'),data )
    connection.undef('dolphindb_share_table','SHARED')
...
parser.add_argument('--table-name', required=True, help='memery share table')
args = parser.parse_args()

conn = create_makrtdatabase()
insert_marketdatabase(args.day_path,conn)
