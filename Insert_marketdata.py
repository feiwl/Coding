...
def create_makrtdatabase():
   ...
   if not s.existsDatabase("dfs://marketdata"):
       s.run("valuep = database(, VALUE, 2018.01.01..2020.12.31)")
...
def insert_marketdatabase(path,connection,share_table,table_name):
    ...
    for file in os.listdir(path):
        ...
        connection.upload({'szWindCode': i[0], "nActionDay":i[1], "nTime": i[2], "nOpen": i[3],
                  "nHigh": i[4], "nLow": i[5], "nMatch": i[6], "iVolume": i[7], "iTurnover": i[8],"nNumTrades":i[9],
                  "bar_close": i[10], "S_DQ_PRECLOSE": i[11], "S_DQ_ADJFACTOR": i[12], "HighLimit": i[13], "LowLimit": i[14]})

        script = "insert into {share_table} values(szWindCode,date(nActionDay),nTime,nOpen,nHigh,nLow,nMatch,iVolume,iTurnover,nNumTrades,bool(bar_close),S_DQ_PRECLOSE,S_DQ_ADJFACTOR,HighLimit,LowLimit);".format(share_table=share_table)
        connection.run(script)
