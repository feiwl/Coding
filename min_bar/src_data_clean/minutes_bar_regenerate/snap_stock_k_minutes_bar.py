...
def get_stocks_k_minute_bar_history(begin_date: str, end_date: str, stock_id: str, K: int) -> pd.DataFrame:
    ...
    """
    target_column = ['szWindCode', 'nActionDay', 'nTime', 'nPreClose', 'nOpen', 'nHigh', 'nLow',
    ...
    sql = """SELECT symbol.szWindCode, snap_min_stock.* FROM marketdata.snap_min_stock join marketdata.symbol 
             where snap_min_stock.codeID = symbol.codeID  and symbol.szWindCode = '%s' 
             and snap_min_stock.nActionDay >= %s and snap_min_stock.nActionDay <= %s;""" \
